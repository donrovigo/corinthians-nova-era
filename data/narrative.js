const Narrative = {
  ...
  names: [
    "um diretor do futebol","um conselheiro experiente","um aliado da sua chapa",
    "um dirigente do clube","um membro do Conselho","um assessor próximo da diretoria"
  ],

  realSimulationNames() {
    const names = [];
    if (typeof POLITICS !== "undefined" && Array.isArray(POLITICS.candidates)) {
      POLITICS.candidates.forEach(person => { if (person?.name) names.push(person.name); });
    }
    if (typeof STAFF !== "undefined" && Array.isArray(STAFF)) {
      STAFF.forEach(person => { if (person?.name) names.push(person.name); });
    }
    return names;
  },

  state() {
    return typeof Game !== "undefined" ? Game.state : null;
  },
  ...
  build(event) {
    ...
    if (realNames.includes(speaker)) base = "SIMULAÇÃO: " + speaker + " — " + base;

    if (["politica","conselho","eleicao"].includes(event.category)) {
      base += c.leak >= 20
    ...
    const choices = (event.choices || []).map(choice => ({
      ...choice,
      text: this.pick(prefixes[tone] || ["Tomar uma decisão"]) + ": " + String(choice.text).toLowerCase(),
  ...
  show(event) {
    ...
    setEvent(
      ...
      generated.choices.map(choice => ({
        text: choice.text,
        action: () => {
          if (typeof applyEventEffects === "function") applyEventEffects(choice.effects);
  ...
  generateConspiracyHint() {
    ...
    const speaker = this.pick(["um conselheiro","um diretor","um aliado","um assessor"]);
    const target = this.pick(["outro diretor","um grupo de conselheiros","um possível aliado","um grupo da oposição"]);
